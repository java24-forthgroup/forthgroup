...
<form class="form-inline" role="form" style="float:left;">
  <div class="form-group has-feedback">
    ...
    <div class="input-group">
      ...
      <input class="form-control has-success" type="text" id="queryByUserName" placeholder="请输入查询条件">
    </div>
  </div>
  <button type="button" id="btnQuery" class="btn btn-warning"><i class="glyphicon glyphicon-search"></i> 查询</button>
</form>
<button type="button" class="btn btn-danger" style="float:right;margin-left:10px;" onclick="delLogs()"><i class=" glyphicon glyphicon-remove"></i> 删除</button>
         ...
         var queryFlag = false;
            $(function () {
			    ...
			    $("#btnQuery").click(function(){
                    var queryName = $("#queryByUserName").val();
			    	if(queryName!=""){
			    		queryFlag = true;
			    	}
			    	queryByPage(1);
            ...
            function delLogs(){
            	layer.confirm("是否删除选中的日志信息?",  {icon: 3, title:'提示'}, function(cindex){
    			    layer.close(cindex);
    			    $.ajax({
    			    	url:"${pageContext.request.contextPath}/accesslog/deleteLogs",
    			    	type:"post",
    			    	data:$("#delForm").serialize(),
    			    	success:function(result){
    			    		if(result.flag){
    			    			layer.msg("删除成功!", {time:1000, icon:0, shift:6}, function(){});
    			    			window.location.href="${pageContext.request.contextPath}/accesslog/index";
    			    		}else{
            ...
            function updateLog(logid){
            	//window.location.href="${pageContext.request.contextPath}/accesslog/updateUI?logid="+logid;
            }

            <!--删除-->
            function deleteLog(logid){
            	layer.confirm("是否删除该日志信息？",  {icon: 3, title:'提示'}, function(cindex){
    			    layer.close(cindex);
    			    $.ajax({
    			    	url:"${pageContext.request.contextPath}/accesslog/delete",
    			    	type:"post",
    			    	data:{"logid":logid},
    			    	success:function(result){
    			    		if(result.flag){
            ...
            function queryByPage(nowPage){
            	...
            	if(queryFlag){
                    jsonData.queryName= $("#queryByUserName").val();
            	}
            	
            	$.ajax({
            		url:"${pageContext.request.contextPath}/accesslog/queryByPage",
            		type:"post",
            		data:jsonData,
            		success:function(result){
            			if(result.flag){
            				var tableStr = "";
            				$.each(result.obj.list,function(index,accessLog){
            					//生成jQuery对象，进行装配或者使用html方法拼字符串
	            				tableStr+="<tr>";
		                        tableStr+="<td>"+(index+1) +"</td>";
                                tableStr+="<td><input type='checkbox' class='delCheck' name='logids' value='"+accessLog.logid+"'></td>";
                                tableStr+="<td>"+accessLog.method+"</td>";
                                tableStr+="<td>"+accessLog.url+"</td>";
                                tableStr+="<td>"+accessLog.ip+"</td>";
                                tableStr+="<td>"+accessLog.userName+"</td>";
                                tableStr+="<td>"+accessLog.addDate+"</td>";
		                        tableStr+="<td>";
		          				tableStr+="<button type='button' onclick='updateLog("+accessLog.logid+")' class='btn btn-success btn-xs'><i class='glyphicon glyphicon-pencil'></i></button>";
		          				tableStr+="<button type='button' onclick='deleteLog("+accessLog.logid+")' class='btn btn-danger btn-xs'><i class='glyphicon glyphicon-remove'></i></button>";
		          				tableStr+="</td>";
	                          	tableStr+="</tr>";
