<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <%@ taglib prefix="yx" uri="http://java.sun.com/jsp/jstl/core" %> 
<html lang="zh-CN">
  <head>
    <meta charset="UTF-8">
...
<button type="button" class="btn btn-danger" style="float:right;margin-left:10px;" onclick="deleteskillgroups()"><i class=" glyphicon glyphicon-remove"></i> 删除</button>
<button type="button" class="btn btn-primary" style="float:right;" onclick="window.location.href='${APPPATH }/Skillgroup/gosave'"><i class="glyphicon glyphicon-plus"></i> 新增</button>
<br>
 <hr style="clear:both;">
	...
	<script src="${APPPATH }/script/docs.min.js"></script>
  	...
  	<script type="text/javascript">
        ...
        function deleteskillgroups(){
            layer.confirm("是否删除这些设备?",  {icon: 3, title:'提示'}, function(cindex){
                layer.close(cindex);
                $.ajax({
                    url:"${APPPATH}/Skillgroup/deletes",
                    type:"post",
                    data:$("#delForm").serialize(),
                    success:function(result){
                        if(result.flag){
                            layer.msg("删除成功!", {time:1000, icon:0, shift:6}, function(){});
                            window.location.href="${APPPATH}/Skillgroup/index";
                        }else{
        ...
        function updateskillgroup(skillgroupId){
            window.location.href="${APPPATH}/Skillgroup/goupdate?skillgroupId="+skillgroupId;
        }

        function deleteskillgroup(skillgroupId) {
            layer.confirm("是否删除该设备?",  {icon: 3, title:'提示'}, function(cindex){
                layer.close(cindex);

                $.ajax({
                    url:"${APPPATH}/Skillgroup/delete",
                    type:"post",
        ...
        function queryByPage(nowPage) {
            $.ajax({
                url:"${APPPATH }/Skillgroup/allSkillgroup",
                type:"post",
                ...
                success:function (result) {
                    if(result.flag){
                        ...
                        $.each(result.obj.list,function (index,skillgroup) {
                            //生成jQuery对象，进行装配或者使用html方法拼字符串
                            tableStr+="<tr>";
                            tableStr+="<td>"+(index+1)+"</td>";
                            tableStr+="<td><input type='checkbox' class='abc' name='skillgroupId' value='"+skillgroup.skillgroupId+"'></td>";
                            tableStr+="<td>"+skillgroup.skillgroupId+"</td>";
                            tableStr+="<td>"+skillgroup.aroom.aroomName+"</td>";
                            tableStr+="<td>"+skillgroup.sourceId+"</td>";
                            tableStr+="<td>"+skillgroup.skillgroupName+"</td>";
                            tableStr+="<td>";
                            tableStr+="<button type='button' onclick='updateskillgroup("+skillgroup.skillgroupId+")' class='btn btn-primary btn-xs'><i class=' glyphicon glyphicon-pencil'></i></button>";
                            tableStr+="<button type='button' onclick='deleteskillgroup("+skillgroup.skillgroupId+")' class='btn btn-danger btn-xs'><i class='glyphicon glyphicon-remove'></i></button>";
                            tableStr+="</td>";
                            tableStr+="</tr>";
                        });
