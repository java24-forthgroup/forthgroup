...
<button type="button" class="btn btn-danger" style="float:right;margin-left:10px;" onclick="deleteequipments()"><i class=" glyphicon glyphicon-remove"></i> 删除</button>
<button type="button" class="btn btn-primary" style="float:right;" onclick="window.location.href='${APPPATH }/Equipment/gosave'"><i class="glyphicon glyphicon-plus"></i> 新增</button>
<br>
 <hr style="clear:both;">
  	...
  	<script type="text/javascript">
        ...
        function deleteequipments(){
            layer.confirm("是否删除这些设备?",  {icon: 3, title:'提示'}, function(cindex){
                layer.close(cindex);
                $.ajax({
                    url:"${APPPATH}/Equipment/deletes",
                    type:"post",
                    data:$("#delForm").serialize(),
                    success:function(result){
                        if(result.flag){
                            layer.msg("删除成功!", {time:1000, icon:0, shift:6}, function(){});
                            window.location.href="${APPPATH}/Equipment/index";
                        }else{
        ...
        function updateequipment(equipmentId){
            window.location.href="${APPPATH}/Equipment/goupdate?equipmentId="+equipmentId;
        }
        ...
        function queryByPage(nowPage) {
            $.ajax({
                url:"${APPPATH }/Equipment/allEquipment",
                type:"post",
                ...
                success:function (result) {
                    if(result.flag){
                        ...
                        $.each(result.obj.list,function (index,equipment) {
                            //生成jQuery对象，进行装配或者使用html方法拼字符串
                            tableStr+="<tr>";
                            tableStr+="<td>"+(index+1)+"</td>";
                            tableStr+="<td><input type='checkbox' class='abc' name='equipmentId' value='"+equipment.equipmentId+"'></td>";
                            tableStr+="<td>"+equipment.equipmentId+"</td>";
                            tableStr+="<td>"+equipment.aroom.aroomName+"</td>";
                            tableStr+="<td>"+equipment.equipmentName+"</td>";
                            tableStr+="<td>"+equipment.equipmentStatus+"</td>";
                            tableStr+="<td>";
                            tableStr+="<button type='button' onclick='updateequipment("+equipment.equipmentId+")' class='btn btn-primary btn-xs'><i class=' glyphicon glyphicon-pencil'></i></button>";
                            tableStr+="<button type='button' onclick='deleteequipment("+equipment.equipmentId+")' class='btn btn-danger btn-xs'><i class='glyphicon glyphicon-remove'></i></button>";
                            tableStr+="</td>";
                            tableStr+="</tr>";
                        });
