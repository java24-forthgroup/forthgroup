				...
				<div class="panel-body">
					<form role="form" id="updateForm">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" id="patientName" name="patientName" value="${patient.patientName}">
						</div>
						<div class="form-group">
							<label 患者年龄></label>
							<input type="text" class="form-control" id="patientAge" name="patientAge" value="${patient.patientAge}">
						</div>
						<div class="form-group">
							<label 患者性别></label>
							<input type="text" class="form-control" id="patientSex" name="patientSex" value="${patient.patientSex}">
						</div>
						<div class="form-group">
							<label 爽约次数></label>
							<input type="text" class="form-control" id="patientCount" name="patientCount" value="${patient.patientCount}">
						</div>
						<div class="form-group">
							<label 患者状态></label>
							<input type="text" class="form-control" id="patientStatus" name="patientStatus" value="${patient.patientStatus}">
						</div>
...
<script type="text/javascript">
	$(function () {
		...
		$("#btnUpdate").click(function(){
			...
			var patientCountVal = $("#patientCount").val();
			if(patientCountVal==""){
				layer.msg("类型名称不能为空!", {time:1000, icon:0, shift:5}, function(){});
				return;
			}
			var patientStatusVal = $("#patientStatus").val();
			if(patientStatusVal==""){
				layer.msg("类型名称不能为空!", {time:1000, icon:0, shift:5}, function(){});
				return;
			}
			$.ajax({
				url:"${APPPATH}/patient/update",
				type:"post",
