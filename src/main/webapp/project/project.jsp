...
<button type="button" class="btn btn-danger" style="float:right;margin-left:10px;" onclick="deleteprojects()"><i class=" glyphicon glyphicon-remove"></i> 删除</button>
<button type="button" class="btn btn-primary" style="float:right;" onclick="window.location.href='${APPPATH }/Project/gosave'"><i class="glyphicon glyphicon-plus"></i> 新增</button>
<br>
 <hr style="clear:both;">
	...
	<script src="${APPPATH }/script/docs.min.js"></script>
  	...
  	<script type="text/javascript">
        ...
        function updateproject(projectId){
            window.location.href="${APPPATH}/Project/goupdate?projectId="+projectId;
        }
        ...
        function queryByPage(nowPage) {
            $.ajax({
                url:"${APPPATH }/Project/allProject",
                type:"post",
                ...
                success:function (result) {
                    if(result.flag){
                        ...
                        $.each(result.obj.list,function (index,project) {
                            //生成jQuery对象，进行装配或者使用html方法拼字符串
                            tableStr+="<tr>";
                            tableStr+="<td>"+(index+1)+"</td>";
                            tableStr+="<td><input type='checkbox' class='abc' name='projectId' value='"+project.projectId+"'></td>";
                            tableStr+="<td>"+project.projectId+"</td>";
                            tableStr+="<td>"+project.skillgroup.skillgroupName+"</td>";
                            tableStr+="<td>"+project.equipment.equipmentName+"</td>";
                            tableStr+="<td>"+project.projectName+"</td>";
                            tableStr+="<td>"+project.projectCost+"</td>";
                            tableStr+="<td>"+project.projectComment+"</td>";
                            tableStr+="<td>";
                            tableStr+="<button type='button' onclick='updateproject("+project.projectId+")' class='btn btn-primary btn-xs'><i class=' glyphicon glyphicon-pencil'></i></button>";
                            tableStr+="<button type='button' onclick='deleteproject("+project.projectId+")' class='btn btn-danger btn-xs'><i class='glyphicon glyphicon-remove'></i></button>";
                            tableStr+="</td>";
                            tableStr+="</tr>";
                        });
